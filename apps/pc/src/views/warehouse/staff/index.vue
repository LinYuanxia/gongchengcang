...
const departmentOptions = ref(['管理部', '采购部', '仓储部', '销售部', '财务部', '质检部'])
const positionOptions = ref(['仓管', '主管', '经理', '采购员', '质检员', '财务'])

const managerOptions = ref([
  { id: '1', realName: '王仓管', position: '仓库主管' },
  { id: '2', realName: '张经理', position: '总经理' },
  { id: '3', realName: '李主管', position: '采购主管' },
])
...
function handleSave() {
  ...
  if (isEdit.value) {
    ...
    const newStaff = {
      id: String(Date.now()),
      staffNo: `W${String(allTableData.value.length + 1).padStart(3, '0')}`,
      ...formData,
