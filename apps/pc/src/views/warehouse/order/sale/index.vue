      ...
      <a-form :model="confirmForm" layout="vertical" style="margin-top: 16px">
        <a-form-item label="审核结果" required>
          <a-radio-group v-model="confirmForm.result">
            <a-radio value="pass">全部通过</a-radio>
            <a-radio value="partial">部分通过</a-radio>
            <a-radio value="reject">全部驳回</a-radio>
          </a-radio-group>
        </a-form-item>
...
function handleConfirmPayment(record: any) {
  ...
  confirmForm.result = 'partial'
  confirmForm.remark = ''
  
  paymentRecords.value.forEach((r: any) => {
    if (r.orderId === record.id && r.status === 'pending') {
      r.auditPass = true
    }
  })
...
function handleConfirmPaymentSubmit() {
  ...
  const orderPaymentRecords = paymentRecords.value.filter(r => r.orderId === currentOrder.value.id)
  const pendingRecords = orderPaymentRecords.filter(r => r.status === 'pending')
  ...
  if (confirmForm.result === 'pass') {
    ...
    Message.success('全部审核通过，订单已变为待发货状态')
  } else if (confirmForm.result === 'reject') {
    pendingRecords.forEach(r => {
      r.status = 'rejected'
    })
    Message.success('全部驳回，订单状态不变')
  } else {
    ...
    if (passedRecords.length > 0) {
      const passedAmount = passedRecords.reduce((sum: number, r: any) => {
        return sum + parseFloat(r.amount.replace(/,/g, ''))
      }, 0)
      
      if (passedAmount >= parseFloat(order.amount.replace(/,/g, ''))) {
        order.paymentStatus = 'confirmed'
        Message.success(`部分审核通过，已确认金额${passedAmount.toLocaleString()}元，订单已变为待发货状态`)
      } else {
        Message.success(`部分审核通过，已确认金额${passedAmount.toLocaleString()}元，剩余金额待确认`)
      }
    } else {
      Message.success('未选择任何凭证通过，订单状态不变')
    }
  }
